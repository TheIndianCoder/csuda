...
import WardwiseDemandGenerationForm from "../updateWaterConsumerModule/WardwiseDemandGenerationForm";
  ...
  async function SearchForWard() {
    ...
    try {
      ...
      var raw = JSON.stringify({
        reportIdentifier: "waste_wardWiseDemandGeneration",
        params: {
          wardID: wardNo,
        },
        paginate: {
      ...
      responseBody?.data[0]?.forEach((data, index) => {
        restructeredData.push({
          ...
          amount: data?.total_demand_amount,
        });
      });
  ...
  return (
    <>
      <WardNumberCollector
        ...
        setoffset={setOffset}
        wardNo={wardNo}
        title={"Ward Wise Demand Generation"}
      />
      {tableLoading && (
...
export default Userwardwisedemand;
